...
export class StandardException extends Error {
  ...
  static hash(str, padstr = '0000000000') {
    const crcstr = '' + StandardException.crc32(str);
  ...
  static generateCode(className, fileName = __dirname) {
    ...
    return parseInt(StandardException.crc32(namespace).toString().substring(0, 5)
      + '000' + StandardException.hash(className));
  }
  ...
  constructor(...args) {
    ...
    if (this.constructor.name !== 'StandardException') {
      ...
      if (!lastArg || false === lastArg.includes(path.sep)) {
